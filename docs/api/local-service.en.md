...
target/debug/areal service status --instance INSTANCE_ID --json
...
| `configFingerprint` | Digest of launch configuration, permissions, deployment files and binary contents |
...
One dataDir allows one Core. `ensure` serializes concurrent launches and checks the running identity/configuration. A mismatch requires an explicit stop or a different dataDir. It never silently expands write/network permissions or kills an unmanaged Core. Symlinks resolve to canonical identity.
...
Compatibility covers effective model/limit configuration, permissions, Runtime budgets, deployment manifests/tool extensions/Workgroup policies and binary contents. Model credential values are excluded from digests and registration. The service inherits its initial environment; credential or other environment changes require explicit restart. Live Provider/Thread configuration remains owned by Core.
...
This version keeps services running until explicit stop, with no idle timer. Default stop checks `restartSafe`, `activeGoals` and `pendingQueueItems`, rejecting outstanding work/resources. `--cancel` uses Core drain to cancel and settle; UNKNOWN or unconfirmed cleanup still prevents success. Accepted stop closes admission; inspect logs/authoritative state after cleanup failure instead of assuming no work happened. Work admitted between the status check and drain follows drain's wait/pause rules.
