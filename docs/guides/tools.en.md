...
Command timeoutMs defaults to 600000, capped by Runtime grants and reported as effectiveTimeoutMs. Commands/continued reads default to 120 seconds and PTYs to 1 second. `yieldMs` / `waitMs` accept nonnegative u64 values; 0 returns immediately. Waits do not extend process deadlines or hold model permits and collect at most `tools.policy.outputPageBytes` bytes (8192 by default). Completed Jest, Karma, Mocha, Cargo test, Pytest and Go test output receives a compact view that preserves failures, summaries and stack lines; the raw page remains available through read_process cursors. Unknown commands pass through unchanged. No output continues waiting; received output is coalesced with 100 ms of silence, using underlying polls of at most 1 second.
